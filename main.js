...
client.on('message', async message => {
	...
	setTimeout(async function () {
        ...
        MongoClient.connect(url, function (err, db) {
          ...
          dbo.collection("levels").find({}).sort({exp: -1}).limit(10).toArray(function (err, output) {
              ...
              output.forEach(user_rank => {
                  User = client.users.get(user_rank.user_id);
                  rankpos = rankpos + 1
                  name = User.username
                  xp = user_rank.exp
                  lvl = user_rank.level
                  user_on_leaderboard += `${rankpos}. ${name} ${tab.repeat(60)} Level ${lvl} ${tab.repeat(2 - (lvl.length)) + "|" + tab.repeat(1 - (lvl.length / 2))} ${xp} XP\n${lines.repeat(52)}\n`
              })
              ...
              dbo.collection("levels").findOne(myquery, async function (err, res) {
                  ...
                  message.channel.send({
                      embed: {
                          ...
                          fields: [{
                                  name: "**Current Level:**",
                                  value: `${curlvl} (Total XP: ${curexp})`,
                              },
                              {
                                  name: "**XP:**",
                                  value: `${difference} / ${rankupxp} (Remaining: ${remaining} XP)\n ${bar} ${count}%`,
                              },
                          ],
                          timestamp: new Date(),
                          footer: {
      ...
      case "levels":
        if (message.author.id != config.owner) return message.reply(`you do not have permission to perform this command!`)
        levels.levelcontrol(); // command code is in commands/channeljoin.js
        break
			default:
